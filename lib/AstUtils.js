...
function getDeclaration(pckg, exportedName, directory = null) {
    ...
    for (const [file, exportDetails] of Object.entries(exports)) {
        ...
        logger.debug(`Found potential file ${file} with exported functions ${[...searchNames].join(", ")}`);
            ...
            if (declarationBox.type === parser.AST_NODE_TYPES.ExportNamedDeclaration) {
                ...
                if (declaration.type === parser.AST_NODE_TYPES.ClassDeclaration ||
                    ...
                    if ((searchNames.has("*") && declaration.id.name === exportedName) ||
                        ...
                        let line = declaration.loc.start.line;
                        logger.debug(`Found matching class for ${exportedName} on line ${line}`);
                        return {
                            ast: ast,
                            declaration: declaration,
                            filePath: file,
    ...
    for (let constructorParam of superClassChain[0].constructorParams) {
        if (constructorParam.type === "complex") {
            ...
            if (superClass !== null) {
                ...
                if (similarParam === null) {
                    logger.error("We could not find a matching argument in a superclass");
                    continue;
                }
                let exportedFields = [];
