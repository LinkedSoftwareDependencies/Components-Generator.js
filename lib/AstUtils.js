...
function getDeclaration(pckg, exportedName, directory = null) {
    let rootFolder = directory == null ? getPackageRootDirectory(pckg) : directory;
    if(rootFolder === null) {
        logger.error(`Could not find root directory of package ${pckg}`);
        return null;
    }
    ...
    try {
        ...
        logger.error(`Could not parse the index file of ${pckg}, invalid syntax at line ${e.lineNumber}, column ${e.column}. Message: ${e.message}`);
        return null;
    }
    ...
    for (const [file, exportDetails] of Object.entries(exports)) {
            ...
            if (declarationBox.type === parser.AST_NODE_TYPES.ExportNamedDeclaration) {
                ...
                if (declaration.type === parser.AST_NODE_TYPES.ClassDeclaration ||
                    ...
                    if ((searchNames.has("*") && declaration.id.name === exportedName) ||
                        ...
                        let line = declaration.loc.start.line;
                        logger.debug(`Found matching class for ${exportedName} on line ${line} in ${file}`);
                        return {
                            ast: ast,
                            declaration: declaration,
                            filePath: file,
        ...
        logger.error(`Did not find a matching exported class in ${file} for name ${exportedName}`);
    }
    return null;
}
...
function getField(property, declaration, imports, nodeModules, commentStart = null) {
    ...
    if (range == null) {
        ...
        if (component !== null) {
            ...
            logger.error(`Could not match field ${fieldName} with any component`);
        }
    }
    let newParameter = {
        "range": range,
...
function getFields(declaration, nodeModules) {
    ...
    console.log(declaration.internalClass);
 ...
 */
function getSuperClassChain(classDeclaration, imports, nodeModules) {
    let superClassChain = [];
    ...
    while (previousSuperClassDeclaration !== null) {
        ...
        let superClassComponent = superClassChain.length === 0 ? null : getComponentByDeclaration(previousSuperClassDeclaration, nodeModules);
    ...
    for (let constructorParam of superClassChain[0].constructorParams) {
        if (constructorParam.type === "complex") {
            ...
            if (similarParam != null) {
                ...
                if (extendsAttribute !== null) newParameter["extends"] = extendsAttribute;
            ...
            if (superClass !== null) {
                ...
                for (let field of fieldData) {
                    ...
                    let parameter = {...{"@id": fieldId}, ...field["parameter"]};
                ...
                let extendsAttribute = getExtendsId(similarParam);
                if (extendsAttribute !== null) newParameter["extends"] = extendsAttribute;
                if (exportedFields.length !== 0) newParameter["fields"] = exportedFields;
                constructorArguments.push(newParameter);
            } else {
                // If the superclass of the parameter is not a constructor parameter of a superclass, we check if
                // it represents a component
                if (constructorParam.component === null) {
                    logger.error(`Found a constructor param ${constructorParam.key} that isn't used in a superclass nor is it a component`);
                    continue;
                }
