...
function getDeclaration(pckg, exportedName, directory = null) {
    ...
    for (const [file, exportDetails] of Object.entries(exports)) {
        ...
        logger.debug(`Did not find a matching exported class in ${file} for name ${exportedName}`);
    }
    return null;
}
...
function getField(property, declaration, imports, nodeModules, commentStart = null) {
    ...
    if (range == null) {
        ...
        if (component !== null) {
            ...
            logger.error(`Could not match class ${constructorReference.className} with any component`);
        }
    }
    let newParameter = {
        "range": range,
